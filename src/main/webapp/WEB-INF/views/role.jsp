    ...
    <form id="roleForm">
        <table class="table table-striped table-bordered table-hover dataTable no-footer" role="grid">
            ...
            <tr>
                ...
                <td>
                    <select id="roleStatus" name="roleStatus" data-placeholder="状态" style="width: 150px;">
    ...
    {{#roleList}}
        <li class="dd-item dd2-item role-name" id="role_{{id}}" href="javascript:void(0)" data-id="{{id}}">
            ...
            <span style="float:right;">
                ...
                &nbsp;
                <a class="red role-delete" href="#" data-id="{{id}}" data-name="{{roleName}}">
...
<script type="text/javascript">
    $(function () {
            ...
            $(".role-edit").click(function (e){
                ...
                $("#dialog-role-form").dialog({
                    ...
                    open: function(event, ui) {
                        ...
                        if(targetRole) {
                            $("#roleId").val(roleId);
                            $("#roleName").val(targetRole.roleName);
                            $("#roleStatus").val(targetRole.roleStatus);
                    ...
                    buttons : {
                        "修改": function(e) {
                            e.preventDefault();
                            createUpdateRole(false, function (data) {
        ...
        // generate permission module and permission ztree
        ...
        function recursivePrepareTreeData(permissionModuleList) {
            ...
            if(permissionModuleList && permissionModuleList.length > 0) {
                $(permissionModuleList).each(function (i, permissionModule) {
                    ...
                    if(permissionModule.permissionList && permissionModule.permissionList.length > 0) {
                        $(permissionModule.permissionList).each(function (i, permission) {
                            zTreeObj.push({
                                id: permissionPrefix + permission.id,
                                pId: modulePrefix + permission.permissionModuleId,
                                name: permission.permissionName + ((permission.permissionType == 1) ? '(菜单)' : ''),
                    ...
                    if((permissionModule.permissionModuleList && permissionModule.permissionModuleList.length > 0) ||
                        (permissionModule.permissionList && permissionModule.permissionList.length > 0)) {
                        nodeMap[modulePrefix + permissionModule.id] = {
                            id: modulePrefix + permissionModule.id,
                            pId: modulePrefix + permissionModule.parentId,
                            name: permissionModule.permissionModuleName,
                            open: hasChecked
                        };
                        var tempPermissionModuleFromNodeMap = nodeMap[modulePrefix + permissionModule.id];
                        while(hasChecked && tempPermissionModuleFromNodeMap) {
                            if(tempPermissionModuleFromNodeMap) {
                                nodeMap[tempPermissionModuleFromNodeMap.id] = {
                                    id: tempPermissionModuleFromNodeMap.id,
                                    pId: tempPermissionModuleFromNodeMap.pId,
                                    name: tempPermissionModuleFromNodeMap.name,
                                    open: true
                                };
                            }
                            tempPermissionModuleFromNodeMap = nodeMap[tempPermissionModuleFromNodeMap.pId];
                        }
                    }
        ...
        // obtain all the selected ids in the ztree
        function getZtreeSelectedId() {
        ...
        $(".role-add").click(function (){
            $("#dialog-role-form").dialog({
                ...
                buttons : {
                    "添加": function(e) {
                        e.preventDefault();
                        createUpdateRole(true, function (data) {
                            $("#dialog-role-form").dialog("close");
        ...
        $(".saveRolePermission").click(function (e) {
            ...
            $.ajax({
                ...
                data:{
                    roleId: lastRoleId,
                    permissionIds: getZtreeSelectedId()
                },
                type: 'POST',
                success: function(result) {
                    if(result.ret) {
        ...
        function createUpdateRole(isCreate, successCallback, failCallback) {
            $.ajax({
